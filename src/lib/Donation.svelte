<script>
  ...
  import { currentScreen, pantriesStore, userStatsStore, preSelectedPantryId } from '../stores.js';
  ...
  // --- CHECK FOR SELECTION ---
  onMount(() => {
    if ($preSelectedPantryId) {
      selectedPantryId = $preSelectedPantryId;
      step = 1; // Skip to Category selection
      preSelectedPantryId.set(null); // Reset
    }
  });
  ...
  function submitDonation() {
    ...
    const summaryString = Object.entries(quantities).map(([name, qty]) => `${name} x${qty}`).join(", ");
    ...
    userStatsStore.update(stats => {
      return {
        ...
        points: stats.points + (totalItems * 10), 
        history: [
          { id: Date.now(), pantry: pantryName, date: "Just now", items: summaryString },
          ...stats.history
        ]
      };
    });
...
<div class="content">
  ...
  {:else if step === 1}
    ...
    <div class="category-grid">
      {#each categories as cat}
        <button class="cat-card" class:selected={selectedCategories.includes(cat.id)} on:click={() => toggleCategory(cat.id)}>
  ...
  {:else if step === 3}
    ...
    <div class="delivery-options">
      <button class="delivery-card" class:selected={deliveryMethod === 'dropoff'} on:click={() => deliveryMethod = 'dropoff'}>
        <div class="d-icon">🚗</div>
        <div class="d-info"><h3>Drop Off</h3><p>I will bring the items.</p></div>
        <div class="radio">{deliveryMethod === 'dropoff' ? '●' : '○'}</div>
      </button>
      <button class="delivery-card" class:selected={deliveryMethod === 'pickup'} on:click={() => deliveryMethod = 'pickup'}>
        <div class="d-icon">🚚</div>
        <div class="d-info"><h3>Request Pickup</h3><p>I need a volunteer.</p></div>
  ...
  {:else if step === 4}
    <div class="success-view">
      ...
      <h2>Thank You!</h2>
      {#if deliveryMethod === 'dropoff'}<p class="sub-text">Please bring your items during open hours.</p>{:else}<p class="sub-text">A volunteer will contact you shortly.</p>{/if}
      <div class="summary-box"><p>Your impact score increased!</p></div>
...
<style>
  ...
  .pantry-select-btn { background: var(--color-ivory); border: 1px solid var(--color-sand); padding: 15px; border-radius: var(--radius-md); display: flex; justify-content: space-between; text-align: left; color: var(--color-espresso); font-weight: bold; }
  ...
  .delivery-card { background: var(--color-ivory); border: 2px solid var(--color-sand); border-radius: var(--radius-md); padding: 20px; display: flex; align-items: center; text-align: left; transition: all 0.2s; }
  .delivery-card.selected { border-color: var(--color-espresso); background-color: #EBE5D9; }
